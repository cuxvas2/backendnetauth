...
public class PeliculasController(IdentityContext context) : Controller
{
    ...
    public async Task<ActionResult<IEnumerable<Pelicula>>> GetPeliculas(string? s)
    {
        if (string.IsNullOrEmpty(s))
            return await context.Pelicula.Include(x => x.Categorias).AsNoTracking().ToListAsync();
        return await context.Pelicula.Include(x => x.Categorias).Where(c => c.Titulo.Contains(s)).AsNoTracking().ToListAsync(); 
    }
    ...
    public async Task<IActionResult> PutPelicula(int id, PeliculaDTO peliculaDTO)
    {
        ...
        var pelicula = await context.Pelicula.FirstOrDefaultAsync(x => x.PeliculaId == id);
        if (pelicula == null) return NotFound();
